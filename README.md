...
It is also, since 2010, a working attack on symmetric cryptography. **Kuwakado and Morii (ISITA 2012)** observed that the Even-Mansour cipher `E(x) = P(x ⊕ k₁) ⊕ k₂` — provably secure to 2^(n/2) classical queries — makes `f(x) = E(x) ⊕ P(x)` periodic with period `k₁`, so the whitening key *is* a hidden period. **Kaplan, Leurent, Leverrier and Naya-Plasencia (CRYPTO 2016)** turned the same observation into forgeries against CBC-MAC, PMAC, GMAC, OCB and GCM. This is the quantum threat to symmetric crypto that Grover does not describe — and one that key length cannot answer: Simon's cost is O(n), so doubling the width roughly doubles the queries instead of squaring them, and no key size restores exponential security.

This demo runs the algorithm as a **full amplitude-level statevector simulation**, accurate to floating-point precision: a real `H⊗ⁿ`, a real unitary XOR oracle, and 2^(n+m) *real* amplitudes in one `Float64Array` — no imaginary part, because no gate in Simon's circuit has one. It could have sampled from the algorithm's known output distribution in one line, and that would have been a lie for most Even-Mansour keys, which collide by accident on top of the period; only an amplitude-level simulation reproduces the skew that causes. Whether a *particular* key does is a fact about that key, so the page computes and prints it per instance rather than asserting it (a complete census over the fixed public permutation finds 2 of the 15 possible k₁ at n = 4 make f exactly 2-to-1). Two of the four targets are real constructions with real consequences: the recovered Even-Mansour key is used to predict a fresh ciphertext, and the recovered CBC-MAC period is used to forge a tag on a message that was never queried, both checked against the real construction.
...
3. **Why at least half the answers vanish** — the interference grid. Every one of the 2ⁿ outcomes is drawn with the sign of its amplitude, before and after the final Hadamard, and the cancelled ones read `0`. Select any outcome and *every* contributing path is computed independently and added in front of you — two for a clean pair, more where the function collided by accident, one for the injective control, where nothing cancels at all: `+1 + −1 = 0` is why that outcome is impossible, not merely unlikely.
...
- **Understanding where Shor came from.** The two share the hidden-period shape — Simon solves the hidden-subgroup problem over (Z₂)ⁿ with Hadamards, Shor's period finding works over cyclic arithmetic groups with the quantum Fourier transform. Simon supplied the conceptual path, but they solve different group structures; Shor is a sibling, not Simon with one gate swapped.
...
- **CBC-MAC, PMAC, GMAC, OCB and GCM.** Kaplan et al. give Simon-based forgeries against all of them. GCM sits in TLS 1.3's one mandatory-to-implement cipher suite (RFC 8446) and is the workhorse AEAD of the deployed web, which is why this result gets cited in quantum-readiness assessments even though it needs Q2 access.
...
- **Post-quantum evaluation criteria.** Superposition-query (Q2) security shows up as an explicit question in modern mode-design papers and standards evaluations largely because of the Kaplan et al. line of work — it is the reason "quantum security" for symmetric crypto means more than Grover's square root.
...
The same spec gates the counters, the failure paths and the page's state discipline. The query tally must split exactly into the equations that raised the rank and the ones that taught nothing, at every stage of a run and not only at the end; the rank must agree across all four surfaces that report it, including the rank meter's screen-reader label; the candidate count must be the `2^(n−rank) − 1` the rank forces rather than a separately tracked number; and the grids' own notes must count the cells actually painted. A candidate `f` refuses must be reported as rejected and named, with nothing claimed recovered while it stands. No verdict may outlive its input: taking a new secret or changing target must retire the verdict, the exploit panel, the equation log and both grids rather than leave a stale one standing, and no control may be left silently dead by a completed run. A break verdict must state the model it holds in — `Broken in the Q2 model only` — and the keyless target's verdict must not. The race is gated on being measured rather than plotted: against the control, whose permutation never collides, the classical mean must equal the domain size exactly (16, 32, 64), and a target switch landing mid-race must neither retarget nor relabel the run already under way.
