 ...
 *
 * WHAT THIS FOUND IN THIS REPO. Unlike most labs in this sweep, the
 * control-boundary token here existed AND was applied: `--border-strong` is the
 * boundary token and `#app .btn` already used it. It was simply too dark to do
 * the job, and two variants overrode it with decorative colours. Five controls
 * measured under 3:1 before the fix that landed in `9fc1c8a`:
 * `--border-strong` itself at 1.79:1 dark and 2.65:1 light; `.btn-secondary`
 * overriding it with `--accent-line` at 1.55:1; and `.btn-primary` and the
 * SELECTED `.seg` segment each painting their border the SAME colour as their
 * own accent fill, so neither had a boundary of its own — 2.36:1 and 2.17:1
 * against the page. That last one is the state that tells a reader which target
 * and which width are active, so it is the one that most needed a boundary.
 *
 * That pass was hand-measurement in a browser. This oracle is what makes it a
 * gate: it runs at EVERY driven state, so it also judges the states no hand
 * measurement reaches — every `.amp-cell` variant the interference grids paint,
 * the `:hover` a reader is left in immediately after clicking, the disabled
 * `#measure` / `#run-all` pair after a run finishes, and the outlined
 ...
 *
 *    This half is inert in this repo today — `src/style.css` declares no
 *    `::before`/`::after` rule of its own, its only mention of either being the
 *    universal selector in the reduced-motion block. It runs anyway, at every
 *    state, so that stays a measurement rather than a reading of the stylesheet.
 *
 ...
 * background beneath. This lab's own palette is flat opaque hex throughout, but
 * the shared top bar's `.cl-btn` edge is a `color-mix(…, transparent)` over the
 * bar's fixed `#0b1512`, and reading the declared value there would measure
 * something the page never paints.
 */
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * Neither branch fires on this page — there is no `gradient` anywhere in
     * `src/style.css` or `index.html` — and it is kept because deleting it would
     * not mean "no gradients" but "gradients silently measured as transparent".
     */
     ...
     * renders ONE `<button class="amp-cell">` PER BASIS STATE, twice, so at
     * n = 6 a single pass judges 128 controls. Without the caches it re-reads the
    ...
    // The paint core above is a verbatim copy of `contrast.ts`'s, so the two
    // oracles cannot drift into measuring different colours for the same
    // pixel. Not every helper in it is reached from this file; `void` marks
    // them used so `noUnusedLocals` does not reject the copy — deleting them
    // instead would fork the core, which is the thing this avoids.
    void splitTopLevel;
    void over;
    void fade;
    void luminance;
     ...
     * what every page paints. One lab in this fleet reports `rgba(0,0,0,0)` for
     * both `html` and `body` because its dark canvas is two radial gradients (not
     * this one, whose `--bg` is a flat hex on both boxes); there the walk
