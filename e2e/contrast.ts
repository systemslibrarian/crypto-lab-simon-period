 ...
 *    an unknown backdrop, or a `color-mix()`. This lab's own palette is
 *    unusually easy on axe: `src/style.css` contains no `gradient`, no `rgba()`
 *    and no `color-mix()` at all, so every surface inside `<main id="app">` is a
 *    flat opaque hex axe can resolve for itself. The shared Crypto Lab top bar
 *    is the exception and it is not decorative — `--cl-ink` is
 *    `color-mix(in srgb, var(--accent) 60%, #eafff8)` and `.cl-btn` draws its
 *    edge from `color-mix(in srgb, var(--cl-ink) 70%, transparent)`, so the
 *    brand title, the Menu / GitHub labels and the theme toggle all land in
 *    axe's `incomplete` bucket rather than being judged. Those are measured here.
 ...
 *    `opacity` declaration, `#app .btn:disabled { opacity: 1 }`, and it is there
 *    deliberately: the disabled state re-colours the ink and the fill instead of
 *    dimming the whole control, so nothing here is faded at all. That is a
 *    property of the current stylesheet, so it is measured rather than trusted —
 *    the first `opacity: .6` added to a muted row would otherwise be invisible to
 *    both oracles.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. There are four
 *    clipping boxes here and every one of them holds real evidence: `.eq-list`
 *    (`overflow-y: auto` under a 21rem cap — the measured equations, which is
 *    the list this lab exists to fill), `.matrix-wrap` and `.table-wrap`
 *    (`overflow-x: auto`), and `.exploit-rows` (`overflow-x: auto`, the
 *    recovered-key check). At 380px all four genuinely scroll. Content scrolled
 *    past that is not dimmed or partly drawn — it is absent, and asking what
 ...
 *    never laid down.
 *
 *  - SVG PAINTS IN DOCUMENT ORDER, SO SIBLINGS CAN BE THE BACKGROUND. Neither
 *    of this lab's figures is SVG: the amplitude grids and the row-reduced
 *    matrix are CSS grids of `<div>`s and `<button>`s, so the sibling walk finds
 *    nothing to do. Its only SVG is the shared top bar's two `aria-hidden`
 *    marks. The branch is kept because the alternative is a helper that silently
 ...
 * not one — nor is it one to axe's `color-contrast` rule. `src/style.css`
 * declares no `::before` or `::after` rule of its own today (its only mention of
 * either is the universal reduced-motion selector), so the class is empty here
 * and stays empty only as long as that holds. `nontext.ts` measures generated
 * content directly at every driven state rather than leaving that to a reading
 * of the stylesheet.
 */
 ...
 * uses. Nothing here is short-lived enough to need the narrowed form: the run
 * animation steps the circuit through its four stages with a real delay, but
 * `main.ts` reads `prefers-reduced-motion` and sets that delay to zero, and this
 * gate asserts the preference is in effect before it scans anything — so every
 * state the drive builds is a settled one.
 *
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * `--cl-ink` and `.cl-btn` edge are both `color-mix()`, and Chromium reports
     * that to `getComputedStyle` unchanged rather than converting it to sRGB. A
     ...
     * page's `body` in the labs that have one: a gradient running the full height
     * of a document several screens tall puts text near the top on a different
     * surface than text near the bottom. So each gradient is *sampled* at the
     * text's real location instead: linear by projecting onto the gradient line,
     ...
     *
     * NEITHER BRANCH FIRES ON THIS PAGE, and that is recorded rather than left
     * to be rediscovered: `src/style.css` and `index.html` contain no `gradient`
     * of any kind, so every `background-image` here resolves to `none` and this
     * function returns TRANSPARENT for every element. It is kept because the
     * fallback if it were deleted is not "no gradients" but "gradients silently
     * measured as transparent", which fabricates a backdrop rather than failing.
     */
     ...
     *
     * The expensive shape here is the pair of amplitude grids: `buildGrid()`
     * renders ONE `<button class="amp-cell">` PER BASIS STATE, twice, so at the
     * widest setting (n = 6) a single scan walks 128 cells each holding two
     * spans, all re-walking the same ancestors up to `<body>`. The row-reduced
     * matrix adds one `<span>` per bit. Without the caches the pass re-reads the
     * same computed styles and rects tens of thousands of times. Nothing mutates
     ...
     * that is `.eq-list` (`overflow-y: auto` under a 21rem cap), `.matrix-wrap`,
     * `.table-wrap` and `.exploit-rows` (`overflow-x: auto`) — between them the
     * measured equations, the row-reduced system, the Grover/Simon/Shor
     * comparison and the recovered-key check.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // would apply the moment this lab draws its circuit or its grids in SVG
      // rather than as the CSS grids of <div>s and <button>s they are today.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // `<details class="aside">` notes — "Why n-1 equations and not n" and "Why
      // the gap looks modest here" — both of which ship shut. The gate opens each
      // by clicking its <summary>, which is the route a reader has, rather than
      // setting `.open` from script, which is what the gate this replaces did to
      // all of them at once before its single scan.
      ...
      // (The OTHER visually-hidden idiom this lab uses, `.sr-only`, is a
      // zero-area clip and is handled by `clippedToNothing` below.)
      ...
      //
      // THIS LAB USES THE IDIOM, so the guard is load-bearing here rather than
      // defensive. `style.css` defines `.sr-only` with exactly the classic
      // recipe — `position:absolute; width:1px; height:1px; clip:rect(0,0,0,0)` —
      // and the page applies it to two headings a sighted reader never sees: the
      // `<h2 id="intro-h">` that names the intro section for the landmark it
      // labels, and the `<caption>` of the Grover/Simon/Shor table. Both keep a
      // 1x1 rect and pass `checkVisibility()` while painting nothing, so without
      // this test they would be measured and reported as failures for ink that
      // was never laid down. It is deliberately narrow: only a ZERO-AREA clip
      // qualifies, so a partially clipped element is still measured, worst case.
     ...
     * lab hides was enumerated and measured by hand rather than assumed. Every
     * one of them is a GLYPH whose meaning is carried in words beside it, which
     * is the only shape of `aria-hidden` that is legitimate here:
     *
     *  - `.verdict-icon` (◌ / ◔ / ✓ / ✗ / ⚠), beside `#verdict-text`, which
     *    spells the same verdict out. Ink is `--text` on the verdict panel.
     *  - the four `.chip` swatches in `.amp-legend` (+, −, 0, ◎), each followed
     *    by the words "amplitude positive", "amplitude negative", "cancelled",
     *    "the outcome measured". They carry the palette, and their ink/fill pairs
     *    are `--accent-text` on `--accent-soft`, `--warn` on `--warn-soft`,
     *    `--text-muted` on `--zero` and `--accent-ink` on `--accent-fill`.
     *  - the ▶ / ◇ marks inside `.break-tag`, whose parent span carries the
     *    words "live above" / "described only".
     *  - the 👁 in the peek warning, beside the sentence it decorates.
     *  - the shared top bar's two SVG marks, which contain no text at all.
     *
     * Nothing on this page hides a VALUE. No amplitude, no measured equation, no
     * recovered period, no query count, no rank and no race row is inside an
     * `aria-hidden` subtree — which was checked rather than assumed. In
     * particular `.amp-cell` sets `aria-label` and `title` on the BUTTON and
     * leaves its `.amp-sign` and `.amp-y` children exposed, so every amplitude
     * on screen is measured by this walk.
     */
     ...
     *
     * This lab has no SVG of its own — the circuit is an `<ol>`, the amplitude
     * grids are `<button>`s and the row-reduced system is a CSS grid of spans.
     * The guard stays because it costs nothing and because the first SVG added would
     ...
     * `style.css` sets `html { background-color: var(--bg) }` as a LONGHAND —
     * the file says so, because axe and WebKit both miss the `background`
     * shorthand — and `--bg` is a flat opaque hex in each theme (`#0d0917` dark,
     * `#f8f6fd` light). So the first branch below returns an opaque colour and
     * the `<body>` fallback is never reached. That is one deleted declaration
     * away from breaking, on a page seven sections and many screens deep whose
     * dark surface is `#0d0917`: fall through to white instead and every element
     * below the first screen would suddenly be measured against a page that does
     * not exist.
     */
